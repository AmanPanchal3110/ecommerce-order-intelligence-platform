...
SELECT
    ...
    i.currency,
    i.event_timestamp
FROM {{ ref('item_silver') }} i
LEFT JOIN {{ ref('payment_silver') }} p ON i.order_id = p.order_id
LEFT JOIN STATUS_TABLE st ON i.order_id = st.order_id 
LEFT JOIN {{ ref('return_silver') }} r ON i.order_id = r.order_id